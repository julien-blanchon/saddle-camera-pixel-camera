...
use saddle_camera_pixel_camera::PixelCameraSystems;

use crate::scenarios;

pub struct PixelCameraLabE2EPlugin;

impl Plugin for PixelCameraLabE2EPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(E2EPlugin);
        app.configure_sets(Update, E2ESet.before(PixelCameraSystems::DetectChanges));
        ...
        if let Some(name) = scenario_name {
            if let Some(mut scenario) = scenarios::scenario_by_name(&name) {
                ...
                error!(
                    "[pixel_camera_lab:e2e] Unknown scenario '{name}'. Available: {:?}",
